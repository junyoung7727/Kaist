...
from main import simplify_circuit_info, calculate_expressibility_from_ibm_results, calculate_entropy_expressibility_from_ibm_results, calculate_quantum_properties
from quantum_base import QuantumCircuitBase, build_qiskit_circuit
...
def generate_all_circuits():
    ...
    # 파라미터 범위 (config 기반 동적 생성)
    qubit_range = circuit_ranges['qubits']
    n_qubits_list = list(range(qubit_range['min'], qubit_range['max'] + 1, qubit_range['step']))
    depth_range = circuit_ranges['depth']
    depth_list = list(range(depth_range['min'], depth_range['max'] + 1))
...
def convert_to_qiskit_circuits(all_circuits, ibm_backend):
    ...
    for i, circuit_data in enumerate(tqdm(all_circuits, desc="회로 변환")):
        try:
            ...
            # 빌드 및 트랜스파일
            qc = build_qiskit_circuit(circuit_info, max_q)
...
def calculate_quantum_properties(circuit_info, qc_transpiled=None):
    ...
    return {
        ...
        "structural_properties": {
            ...
            "single_qubit_gates": sum(gate_counts.get(g, 0) for g in ["H", "X", "Y", "Z", "S", "T", "RZ", "RX", "RY"]),
...
def create_circuit_embedding(circuit_sequence, max_length=100):
    ...
    # 게이트 타입을 숫자로 매핑
    gate_to_id = {
        "H": 1, "X": 2, "Y": 3, "Z": 4, "S": 5, "T": 6, "RZ": 7, "RX": 8, "RY": 9, "CNOT": 10, "CZ": 11,
        "PAD": 0  # 패딩용
    }
...
def analyze_two_qubit_ratio_results(all_results):
    ...
    for ratio in [0.1, 0.3, 0.5]:
        ratio_results = []
        for result in all_results:
            if abs(result["circuit_info"]["two_qubit_ratio"] - ratio) < 0.01:
