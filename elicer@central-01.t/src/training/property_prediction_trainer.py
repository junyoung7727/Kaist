...
class PropertyPredictionDataset:
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Handle missing robust_fidelity
        robust_fidelity = measurement.robust_fidelity if measurement.robust_fidelity is not None else measurement.fidelity * 0.8
...
def collate_fn(batch: List[Dict]) -> Dict:
    ...
    # 타겟 값들을 텐서로 변환
    targets = {}
    for key in ['entanglement', 'fidelity', 'robust_fidelity', 'expressibility']:
...
class PropertyPredictionTrainer:
    ...
    def __init__(
        ...
        self.criterion = PropertyPredictionLoss(
            ...
            robust_fidelity_weight=getattr(config, 'weight_robust_fidelity', 1.0),
    ...
    def _debug_predictions_vs_targets(self, predictions: Dict[str, torch.Tensor], 
        ...
        for property_name in ['entanglement', 'fidelity', 'robust_fidelity', 'expressibility']:
        ...
        if 'combined' in predictions and 'combined' in targets:
            ...
            property_names = ['entanglement', 'fidelity', 'robust_fidelity', 'expressibility']
    ...
    def _calculate_metrics(self, predictions: List[Dict], targets: List[Dict]) -> Dict[str, float]:
        ...
        try:
            ...
            # 분석할 프로퍼티 지정
            properties = ['entanglement', 'fidelity', 'robust_fidelity', 'expressibility']
...
def create_datasets(data_path: str, train_ratio: float = 0.7, val_ratio: float = 0.15) -> Tuple[PropertyPredictionDataset, PropertyPredictionDataset, PropertyPredictionDataset]:
    """merged_data.json을 사용한 데이터셋 분할 생성"""
...
if __name__ == "__main__":
    ...
    config = PropertyPredictionConfig(
        d_model=256,
        n_heads=8,
        n_layers=4,  # 메모리 효율성을 위해 레이어 수 감소
        dropout=0.1,
        learning_rate=1e-4,
        property_dim=4  # entanglement, fidelity, robust_fidelity, expressibility
    )
    ...
    try:
        ...
        # Start training
        trainer.train(num_epochs=50)  # 적은 에폭으로 시작
        
    except FileNotFoundError:
        print(f"❌ 데이터 파일을 찾을 수 없습니다: {data_path}")
        print("merged_data.json 파일 경로를 확인해주세요.")
    except Exception as e:
