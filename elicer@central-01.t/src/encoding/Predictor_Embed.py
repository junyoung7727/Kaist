...
class QuantumCircuitEmbedding(nn.Module):
    ...
    def forward(self, encoded_circuit: Dict) -> Dict[str, torch.Tensor]:
        ...
        """
        # 모델 가중치의 device 가져오기
        device = self.gate_embedding.weight.device
        
        if not encoded_circuit.get('nodes'):
            return {
                'node_embeddings': torch.zeros(0, self.d_model, device=device),
                'circuit_embedding': torch.zeros(self.d_model, device=device)
            }
        ...
        if self.use_rotary_pe and len(node_embeddings) > 0:
            ...
            grid_structure = self._create_grid_structure(encoded_circuit)
        return {
            'node_embeddings': node_embeddings,
            'circuit_embedding': torch.mean(node_embeddings, dim=0) if len(node_embeddings) > 0 else torch.zeros(self.d_model, device=device),
    ...
    def _embed_nodes(self, nodes: List[Dict]) -> torch.Tensor:
        ...
        if not nodes:
            return torch.zeros(0, self.d_model, device=self.gate_embedding.weight.device)
        ...
        for node in nodes:
            ...
            gate_idx = self.gate_vocab.get(gate_name)
            gate_emb = self.gate_embedding(torch.tensor(gate_idx, device=device))  # [gate_dim]
            
            # 2. 역할 임베딩 (RoPE가 positional encoding 담당)
            role = node.get('role', 'single')
            role_dict = {'single': 0, 'control': 1, 'target': 2, '[PAD]': 3}
            role_emb = self.role_embedding(torch.tensor(role_dict.get(role, 0), device=device))  # [role_dim]
            
            # 3. 파라미터 임베딩 (단순화된 버전)
            param_val = node.get('parameter_value', 0.0)
            # 스칼라 입력을 명시적으로 1차원으로 만들어서 Linear 레이어에 전달
            param_input = torch.tensor(param_val, dtype=torch.float, device=device).unsqueeze(0)  # [1]
            ...
            param_indicator_scalar = torch.tensor([has_param], device=device)  # [1] scalar tensor
    ...
    def _create_grid_structure(self, encoded_circuit: Dict) -> Dict[str, torch.Tensor]:
        ...
        if not nodes:
            return {
                'positions': torch.empty(0, 2, device=device),
                'distance_matrix': torch.empty(0, 0, device=device),
        ...
        # 그리드 포지션 매트릭스
        positions = torch.tensor([node['grid_position'] for node in nodes], device=device)
        
        # 거리 매트릭스 (연결도 기반 hop 수)
        dist_matrix = self._compute_hop_distance_matrix(nodes, encoded_circuit['edges'], node_to_idx, device)
    ...
    def _compute_hop_distance_matrix(self, nodes: List[Dict], edges: List[Dict], node_to_idx: Dict, device=None) -> torch.Tensor:
        """
        ...
        # torch 텐서로 변환
        return torch.tensor(dist_matrix, dtype=torch.float32, device=device)