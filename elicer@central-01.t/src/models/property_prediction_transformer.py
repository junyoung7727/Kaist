...
class PropertyPredictionConfig:
    ...
    n_heads: int = 8
    n_layers: int = 6
    d_ff: int = 1024  # Feed-forward dimension (Decision Transformer 호환)
    dropout: float = 0.1
    ...
    # Output settings
    property_dim: int = 4  # entanglement, fidelity, robust_fidelity, expressibility
...
class PropertyPredictionHead(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1):
        ...
        def create_stable_head(input_dim):
            return nn.Sequential(
                nn.Linear(input_dim, input_dim // 2),
                nn.LayerNorm(input_dim // 2),
                nn.GELU(),
                nn.Dropout(dropout * 0.3),  # 낮은 dropout
                nn.Linear(input_dim // 2, 1),
                nn.Tanh()  # 출력 범위 제한 (-1, 1)
            )
        
        self.entanglement_head = create_stable_head(d_model // 2)
        self.fidelity_head = create_stable_head(d_model // 2)
        self.robust_fidelity_head = create_stable_head(d_model // 2)
        self.expressibility_head = create_stable_head(d_model // 2)
        ...
        self.combined_head = nn.Sequential(
            ...
            nn.Linear(d_model // 4, 4),  # [entanglement, fidelity, robust_fidelity, expressibility]
            nn.Tanh()  # 출력 범위 제한 (-1, 1)
        )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        predictions = {
            ...
            'combined': self.combined_head(shared_features)
        }
        ...
        for key, value in predictions.items():
            if torch.isnan(value).any() or torch.isinf(value).any():
                ...
                predictions[key] = torch.clamp(value, -0.99, 0.99)  # 안전한 범위
        
        return predictions


class TransformerBlock(nn.Module):
    """트랜스포머 블록 (Decision Transformer 아키텍처 호환)"""
    
    def __init__(self, config: PropertyPredictionConfig):
        super().__init__()
        self.config = config
        
        # Decision Transformer와 호환되는 ModularAttention 사용
        ...
        self.attention = create_modular_attention(
            ...
            mode="ADVANCED"  # Decision Transformer와 동일한 모드
        )
...
class PropertyPredictionTransformer(nn.Module):
    ...
    def _forward_single(self, circuit_spec) -> Dict[str, torch.Tensor]:
        """단일 회로 처리 (Decision Transformer 호환)"""
        # 1. Grid encoding
        ...
        distance_matrix = grid_structure['distance_matrix']  # [seq_len, seq_len]
        edges = encoded_circuit['edges']
        
        # 3. Decision Transformer 표준 차원으로 변환: [1, seq_len, d_model]
        x = node_embeddings.unsqueeze(0)  # [1, seq_len, d_model]
        
        # 4. Attention mask 생성 (Decision Transformer 표준)
        seq_len = x.size(1)
        attention_mask = torch.ones(1, seq_len, seq_len, device=x.device)  # [1, seq_len, seq_len]
        
        # 5. Transformer layers (Decision Transformer 호환)
        for layer in self.transformer_layers:
            x = layer(x, attention_mask, grid_structure, edges, device=x.device)
        
        # 6. Global pooling for circuit representation
        # x: [1, seq_len, d_model] -> [1, d_model]
    ...
    def _forward_batch_with_padding(self, circuit_specs: List[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for circuit_spec in circuit_specs:
            ...
            embeddings_list.append(embedding_result['node_embeddings'])
            grid_structures.append(embedding_result['grid_structure'])
            edges_list.append(encoded_circuit['edges'])
            seq_lengths.append(embedding_result['grid_structure']['seq_len'])
        ...
        for layer in self.transformer_layers:
            # 모든 배치를 한 번에 처리 (Decision Transformer 방식)
            x = layer(x, attention_masks, grid_structures[0], edges_list[0], device=x.device)
...
class PropertyPredictionLoss(nn.Module):
    ...
    def forward(
        ...
        if 'expressibility' in targets:
            losses['expressibility'] = self.mse_loss(
